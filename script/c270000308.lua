...
function s.initial_effect(c)
	...
	e2:SetOperation(function(e,tp,eg,ep,ev,re,r,rp) if Duel.NegateEffect(ev) then Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP) end end)
	c:RegisterEffect(e2)
...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	local ch=ev-1
	if ch==0 or not (ep==1-tp and Duel.IsChainDisablable(ev)) or re:GetHandler():IsDisabled() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then return false end
